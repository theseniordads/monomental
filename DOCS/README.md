...
One thing that made us laugh was that in another section of the Ripper, we found a section for searching for Atomik packed data, and saving it unpacked! We could have just used the Adrenalin Ripper to unpack our PI3 pics!

Of course, we had to go through the same rigmarole as with the pictures when it came to slotting them into the source code. Fortunately the music files are only referenced a couple of times in the code each, so it was eaasier to fix the labels to meaningful ones. Also, the music files are next to each other in the code, which made it much easier to replace the source with `incbin` replacements.
...
We were seeing a lot of reference errors, where something in the code is pointing to a label Vlink can't find. Oddly enough, all these references were to local labels
